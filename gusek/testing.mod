...
var x{i in H}, binary; #assignment

#############################################
minimize Cost: (sum{i in H} x[i]); 
#minimize Cost: (sum{i in 1..n} x[i]); 
#############################################
subject to first {j in 1..m}: (sum{i in H} S[i,j]*x[i])>=y[j];

solve;
printf "\n";
#printf{i in 1..n} "%d,", x[i]; changed 12.07
printf{i in H} "%d,", x[i];
table result{i in H} OUT "CSV" "cov_node.csv":
 i~i, x[i]~x;
/* Displaying results */
#display x{i in 1..n};
...
param n:= 1000;  # number of sensors
